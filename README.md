...
- **一键开关**：侧边栏底部（设置按钮旁）的终端按钮打开/收起面板；默认支持 `Ctrl+Shift+\`` 快捷键（DevTools 打开时也生效），可在设置中关闭或录制为任意自定义组合键。
...
| 操作 | 位置/快捷键 |
| --- | --- |
| 打开/收起终端面板 | 侧边栏底部终端按钮；快捷键默认 `Ctrl+Shift+\``（可在设置中自定义） |
...
- **字号**：10–24px。
- **快捷键**：默认 `Ctrl+Shift+\`` 开关面板；点“更改…”录制自定义组合键（需包含 Ctrl/Alt/Meta），可恢复默认或整体关闭。
